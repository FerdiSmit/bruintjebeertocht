 ...
 * and process the entries one at a time, with a delay between
 * each one. The idea is to make brute-force attacks on the
...
class LoginAttempt
{
    ...
    /**
     * @var int The number of milliseconds before an unchecked attempt is considered dead.
     */
     ...
     * As it may be used multiple times per attempt, it makes sens not to initialize
     * it each ready check.
     *
     * @var PDOStatement
     */
    ...
    public function __construct($email, $password, \PDO $pdo)
    {
        ...
        {
            throw new Exception('De grote van de wachtrij is overschreden.', 503);
        }
    }
    ...
    private function addToQueue()
    {
        ...
        {
            throw new Exception('Het IP adres staat al in de wachtrij.', 403);
        }
    }
    ...
    private function isQueueSizeExceeded()
    {
        ...
        if (!$count)
        {
            throw new Exception('Het is niet gelukt de grootte van de wachtrij op te vragen.', 500);
        }
    ...
    /**
     * Checks if the login attempt is ready to be processed, and updated the
    ...
    /**
     * Checks is the login attempt is valid. Note that this function will cause
    ...
    public function isValid()
    {
        if ($this->isLoginValid === null)
        {
            ...
            $sql = 'DELETE FROM login_attempt_queue
                    WHERE id = ?
                    OR last_checked < NOW() - INTERVAL ? MICROSECOND';
            $stmt = $this->pdo->prepare($sql);
            $stmt->execute(array(
                $this->attemptID,
                self::ATTEMPT_EXPIRATION_TIMEOUT * 1000
